...
def informationExtractor(sym):
    ...
    #df3.to_csv('Check_Data.csv')
    return df3

def dataprocessor():
    filename = 'NSE_Circuit_Jump'+times+'.csv'
    with open(filename, 'w', newline='') as csvfile:
        fields = ['Percentage', '5%', '8%', '10%','12%','15%','18%','20%']
        ...
        for sym in NSE2:
            ...
            rows = [[sym, count5, count8, count10, count12, count15, count18, count20]] 
            StockCodesAngelOne.objects.create( token = (row[0]), stock_Name =  (row[1]), Name= (row[2]),Market=  (row[3]), Category= 'Unknown')       
